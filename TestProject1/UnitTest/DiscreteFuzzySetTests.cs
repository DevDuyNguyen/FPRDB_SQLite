...
namespace TestProject1.UnitTest
{
    public class DiscreteFuzzySetTests
    {
        ...
        class StandardIntersection_positive_testdata:TheoryData<FuzzySet<float>, FuzzySet<float>, FuzzySet<float>>
        {
            ...
            {
                Add(
                    new DiscreteFuzzySet<float>(new List<float> { 1.1f, 3.2f, 1.3f}, new List<float> { 0.2f, 0.2f, 0.3f, 1f}, "fs1", FieldType.DIST_FUZZYSET_FLOAT, -1),
                    new DiscreteFuzzySet<float>(new List<float> { 1.1f, 1.2f, 4 }, new List<float> { 0.1f, 0.2f, 0.3f, 1f }, "fs2", FieldType.DIST_FUZZYSET_FLOAT, -1),
                    new DiscreteFuzzySet<float>(new List<float> { 1.1f, 3.2f, 1.2f, 1.3f, 4 }, new List<float> { 0.1f, 0, 0, 0,0 }, "fs1⋂fs2", FieldType.DIST_FUZZYSET_FLOAT, -1)
                    );
            }
        ...
        class isEqualTo_postive_negative_testdata:TheoryData<FuzzySet<float>, FuzzySet<float>, bool>
        {
            public isEqualTo_postive_negative_testdata()
            {
                Add(
                new DiscreteFuzzySet<float>(new List<float> { 1.1f, 3.2f, 1.3f }, new List<float> { 0.2f, 0.2f, 0.3f, 1f }, "fs1", FieldType.DIST_FUZZYSET_FLOAT, -1),
                new DiscreteFuzzySet<float>(new List<float> { 1.1f, 1.2f, 4 }, new List<float> { 0.1f, 0.2f, 0.3f, 0.2f }, "fs2", FieldType.DIST_FUZZYSET_FLOAT, -1),
                false
                );
        ...
        class isSubsetOf_postive_negative_testdata : TheoryData<FuzzySet<float>, FuzzySet<float>, bool>
        {
            public isSubsetOf_postive_negative_testdata()
            {
                ...
                Add(
                new DiscreteFuzzySet<float>(new List<float> { 1.1f, 3.2f, 1.3f }, new List<float> { 0.2f, 0.2f, 0.3f, 1f }, "fs1", FieldType.DIST_FUZZYSET_FLOAT, -1),
                new DiscreteFuzzySet<float>(new List<float> { 1.1f, 3.2f, 1.3f }, new List<float> { 0.2f, 0.3f, 0.3f, 1f }, "fs1", FieldType.DIST_FUZZYSET_FLOAT, -1),
                true
                );
